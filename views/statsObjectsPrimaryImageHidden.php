...
//megahaluz, několikrát vnořený dotaz co získá seznam objektů, které mají primární obrázek neviditelný, ale mají nějaký, který je omezený nebo veřejný
  ...
  $SQL= "
  select  ca_objects.object_id, ca_objects.idno,  min(ca_objects_x_object_representations.representation_id) as obrazek from 
...
where ca_object_representations.access<>1435 and
...
group by object_id having count(*) >1) group by object_id
  ...
  foreach($dotaz  as $Row){
    ...
    $t_media = new ca_object_representations( $Row["obrazek"]);
    $m=$r->getRepresentationMediaForIDs(array( $Row["obrazek"]),array("preview", "small", "medium", "large")  );
    //var_export($m[$Row1["representation_id"]]['urls']);
    $p= $m[$Row1["representation_id"]]['urls'];

    echo "<img src='". $p['preview']."'>";
