...
left join ca_attribute_values on ca_attributes.attribute_id=ca_attribute_values.attribute_id
 WHERE `table_num` = 57 and ca_attributes.element_id=2 and row_id= '.$id;
    ...
    $data[$rep][]= "INSERT INTO `victim_images` ( `preview`, `small`, `medium`, `large`, `type`, `victim_id`, `object_id`, `idno`, `about`) SELECT '".$p['preview']."','".$p['small']."','".$p['medium']."','".$p['large']."',2, id, ".$id.", null, null FROM victim where entity_id in(".$ids_e.") or id_external  in(".$ids_e.");"."<br>/*".$m[$Row1["representation_id"]]['tags']['preview']."*/<br><br>";
  }
